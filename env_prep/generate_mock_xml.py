...
class MockXMLGenerator:
    ...
    def _generate_mock_xml(self, app_id: int) -> str:
        ...
        # Generate interesting contact ID
        con_id = app_id * 2 # random.randint(1, 80000)
        tinyint = random.randint(1, 255)
        smallint = random.randint(1, 32766)
        
        # Generate contacts XML
        contacts_xml = self._generate_contacts_xml(con_id)
        
        # Template XML structure
        xml_template = f'''<Provenir TestType="MOCK_APP_{app_id}" GeneratedOn="{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}">
            <Request ID="{app_id}" Process="20000" Priority="Alloy Error" Status="B" LastUpdatedBy="bob-{app_id}" _LockedBy="MOCK_APP_{app_id}@TEST.COM" LockedAt="{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}">
                <CustData>
                    <application app_type_code="PRODB" app_receive_date="2023-11-21 8:46:18.{app_id}" last_update_time="{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}" signature_ind="N" name_match_flag="Y" address_match_flag="N" ssn_match_flag="R" pricing_tier="59" solicitation_num="MOCK_{app_id}T" pymnt_prot_plan_ind="Y" credit_life_ind="N" campaign_num="T{app_id}" swiftpay_num="13{app_id}" mail_cell_pricing="XJEFH8E171" Partner_URL_ID="mock-test-app_id-{app_id}@bro.com" app_source_ind="I" Debit_Refund_Failed="Y" Debit_Initial_Deposit_date="10/11/2024" Debit_Initial_Deposit_amount="322.{app_id}" Debit_NSF_Return_date="10/12/2024" Debit_Refund_amount="{tinyint}.{app_id}" Debit_Refund_date="{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}" Debit_Funding_Source="ACH" special_offer="3" population_assignment="CM" verification_source="TLO" secure_ach_amount="{tinyint}.{app_id}" IP_address="10.20.138.{app_id}" EXPinNumber="{app_id}" assignedTo="test-{app_id}@testy.com" esign_consent_flag="false" paperless_flag="true" secure_ach_sent_flag="N">
                        <app_product decision_tp_c="APPRV" duplicate_app_ind="Y" decision_date="{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}" Risk_Model_reason1_tp_c="RISK_{app_id}" Risk_Model_reason2_tp_c="RISK_{app_id}" Risk_Model_reason3_tp_c="RISK_{app_id}" Risk_Model_reason4_tp_c="" adverse_actn1_type_cd="AA_{app_id}" adverse_actn2_type_cd="" adverse_actn3_type_cd="AA_{app_id}" adverse_actn4_type_cd="" adverse_actn5_type_cd="" debt_to_income_ratio="9.{app_id}" booked_date="{datetime.now().strftime('%m/%d/%Y')}" multran_booked_date="{datetime.now().strftime('%m/%d/%Y')}" supervisor_rev_ind="N" analyst_rev_ind="Y" monthly_debt="573.{app_id}" monthly_income="6250.{app_id}" precision_score="0" experian_fico_score="0" prescreen_fico_score="{app_id}" prescreen_risk_score="{app_id}" risk_model_score="" prescreen_risk_grade="E" prescreen_fico_grade="D" backend_risk_grade="U" backend_fico_grade="F" Vantage_Score_TU="" Vantage_Score_EX="" risk_model_score_JH="" risk_model_score_LP="" TU_DIE_score="" TU_TIE_score="" classic_08_score="" risk_model_score_JB="" VeridQA_Result="TEST" InstantID_Score="50" disclosures="N" EX_FICO_08_score="610" EX_EVS_score="0" EX_TIE_score="29" EX_DIE_score="56" TU_L2C_score="0" GIACT_Response="" TU_FICO_09_Score="" fraud_rev_ind="Y" bank_debt_to_income_ratio="19.{app_id}" regb_start_date="2023-9-21 8:46:46.{app_id}" regb_end_date="2023-12-21 8:18:18.{app_id}" booking_paused="Y" decision_model="EX FICO 08" decision_score="610" DIE_Plus_Score="78.{app_id}" />
                        <rmts_info campaign_number="X4J" name_match_flag="N" ssn_match_flag="N" address_match_flag="Y" primary_first_name="MARKY-MOCK-{app_id}" primary_middle_name="G" primary_last_name="WOODFIELD" primary_suffix="" primary_ssn="{app_id}" pri_cur_street_num="{app_id}" pri_cur_street_name="S {app_id} HWY" pri_apt_number="{app_id}" pri_rural_rt_num="{tinyint}" pri_po_box_num="12" pri_city="HORSE CAVE - {app_id}" pri_state="KY" pri_zip_code="{app_id}" special_flag_5="A" special_flag_6="G" special_flag_7="A" special_flag_8="Z" aaa_misc_field_4="Wah" CB_prescreen_birth_date="05/10/1978" auth_user_is_spouse="Y" minimum_interest_charge="1.{app_id}" account_setup_fee="{tinyint}" additional_card_fee="{tinyint}" late_payment_fee="Up to $40" over_limit_fee="{tinyint}" returned_payment_fee="{tinyint}" apr="0.2{app_id}" annual_fee="{tinyint}" cash_advance_apr="2.{app_id}" foreign_percent="4.{app_id}" cash_advance_fee="36.{app_id}" cash_advance_percent="9.{app_id}" cash_apr_margin="7.{app_id}" intro_cash_advance_apr="14.{app_id}" intro_purchase_apr="18.{app_id}" min_payment_percent="4.{app_id}" purchase_apr_margin="13.{app_id}" seg_plan_version="{tinyint}" min_payment_fee="{tinyint}" />
                        {contacts_xml}
                    </application>
                </CustData>
            </Request>
    ...
    def _generate_contacts_xml(self, con_id: int) -> str:
        ...
        contact_xml = f'''<contact con_id="{con_id}"  ac_role_tp_c="PR" sms_consent_flag="false" first_name="TOMMY-{con_id}" initials="G" last_name="BARKER" suffix="" ssn="{con_id}" email="tommy-{con_id}@mock.com" birth_date="05/10/1973" mother_maiden_name="DAVIS" fraud_ind="S" banking_aba="19{con_id}" banking_account_number="90{con_id}" banking_account_type="C" >
          <app_prod_bcard card_decision_tp_c="APPRV" pd_tp_c="X4J" requested_credit_line="{con_id}" allocated_credit_line="1{tinyint}" accnt_type_ind="" account_number="{con_id}" multran_account_number="{con_id}" min_pay_due="{tinyint}.{con_id}" credit_line2="1{tinyint}" max_line="2{tinyint}"/>
          <contact_address address_tp_c="CURR" residence_monthly_pymnt="{tinyint}.{con_id}" city="MOCK CITY - {con_id}" rural_route="RR#1" state="KY" po_box="{con_id}" street_name="JACKSON HWY" street_number="{con_id} S" unit="12" home_phone="801{con_id}" cell_phone="401{con_id}" address_line_1="LOTS" zip="{con_id}" months_at_residence="11" years_at_residence="2" ownership_tp_c="O" />
          <contact_employment employment_tp_c="CURR" b_phone_no="101{con_id}" b_addr_1="LINE ONE - {con_id}" b_unit="#c-30" b_zip="10{con_id}" b_state="WA" b_street_name="SNAME" b_street_number="{con_id} W" b_city="THE BIG CITY" b_other_income_amt="{tinyint}" b_other_income_source_detail="other source detail" b_name="MOCK-STYLE {con_id}" b_job_title_tp_c="BIG GUY" b_income_source_nontaxable="F" b_self_employed_ind="Y" b_months_at_job="6" b_years_at_job="1" b_primary_income_source_tp_c="INVEST" b_salary="{con_id}" b_salary_basis_tp_c="ANNUM" b_othr_inc_basis_tp_c="WEEK" b_other_income_source_tp_c="BONUS" />
        </contact>'''
        return contact_xml

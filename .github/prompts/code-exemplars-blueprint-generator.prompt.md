...
${PROJECT_TYPE="Auto-detect|.NET|Java|JavaScript|TypeScript|React|Angular|Python|SQL|JSON|Other"} <!-- Primary technology -->
...
- In this repo, include exemplars for Python ETL logic, JSON mapping contracts, and SQL query patterns where applicable
...
${PROJECT_TYPE == "SQL" || PROJECT_TYPE == "Auto-detect" ? `#### SQL Exemplars (if detected)
- **Query Formatting**: Readable, aligned SQL with clear clauses
- **Parameterized Queries**: Examples using parameters (no string concatenation)
- **Join Patterns**: Clear join syntax and filtering
- **Validation Queries**: Duplicate detection or integrity checks
- **Transaction Scope**: Queries used in atomic operations` : ""}

${PROJECT_TYPE == "JSON" || PROJECT_TYPE == "Auto-detect" ? `#### JSON Exemplars (if detected)
- **Contract Files**: Mapping contracts with consistent structure
- **Schema-Like Sections**: Clear type and nullability usage
- **Enum Mappings**: Readable enum mapping patterns
- **Calculated Fields**: Consistent calculated field definitions
- **Validation Rules**: Contract validation patterns` : ""}
